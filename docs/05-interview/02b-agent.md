...
**基础概念**
[Q1 如何定义 Agent？核心组件有哪些？](#q1如何定义一个基于-llm-的-agent核心组件有哪些) · [Q2 ReAct 框架详解](#q2请详细解释-react-框架它如何将思维链和行动结合) · [Q3 Agent 规划能力实现方式](#q3agent-的规划能力有哪些实现方式cot--tot--got-各有何特点)

**Memory 系统**
[Q4 短期/长期记忆系统设计](#q4如何为-agent-设计短期记忆和长期记忆系统) · [Q5 记忆系统实战追问](#q5你是怎么设计-agent-的记忆系统长期记忆如何存储历史记录量很大时怎么优化) · [Q6 记忆衰退设计](#q6有没有做记忆衰退避免旧数据干扰新任务) · [Q7 向量数据库与检索策略](#q7为什么-agent-记忆系统常用向量数据库如何设计-embedding-和检索策略) · [Q8 Function Calling 原理](#q8llm-是如何学会调用外部-api-或工具的从-function-calling-角度解释) · [Q9 工具调用失败处理](#q9你的-agent-如何处理工具调用失败api-超时返回空有无重试降级机制) · [Q10 稳定 JSON 输出](#q10问数据的输入输出格式如何保证大模型输出稳定的-json) · [Q11 Tool 设计与 Workflow](#q11agent-的工具-tool-设计是否是-workflow-形式)

**Multi-Agent 协作**
[Q12 多智能体系统优劣](#q12什么是多智能体系统相比单-agent-有什么优势又引入哪些新复杂性) · [Q13 多 Agent 协同机制](#q13如何让多个-agent-协同工作举一个具体的协同机制例子) · [Q14 策略冲突处理](#q14如果一个-agent-误判导致策略冲突如何处理) · [Q15 A2A 框架](#q15了解-a2a-框架吗和普通-agent-框架最关键的不同是什么)

**挑战与安全**
[Q16 构建复杂 Agent 的主要挑战](#q16构建复杂-agent-最主要的挑战是什么) · [Q17 行为安全与可控](#q17如何确保-agent-行为安全可控且符合人类意图) · [Q18 隐私与越权防护](#q18如何防止-agent-泄露用户隐私或越权操作从算法和系统层面谈谈设计)

**系统设计真题**
[Q19 旅行规划 Agent 设计](#q19如果让你设计一个行程规划旅行-agent如何拆解任务各子-agent-职责怎么划分) · [Q20 决策链可解释性](#q20如果用户连续追问为什么选这家酒店agent-如何回溯决策链给出可解释理由) · [Q21 鲁棒性评估](#q21如何评估一个-agent-系统的鲁棒性除了准确率还会测试哪些对抗性-case) · [Q22 Bad Case 收集与迭代](#q22项目上线后如何收集-bad-case-并迭代有做在线学习吗)
