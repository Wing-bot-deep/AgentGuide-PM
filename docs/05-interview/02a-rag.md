...
**核心原理**
[Q1 RAG 工作原理与 Fine-tune 对比](#q1请解释-rag-的工作原理与直接-fine-tuning-相比rag-主要解决了什么问题) · [Q2 完整 RAG 流水线](#q2一个完整的-rag-流水线包含哪些关键步骤从数据准备到最终生成) · [Q3 Chunking 策略选择](#q3文本切块chunking策略如何选择切块大小和重叠长度背后有什么权衡) · [Q4 Embedding 模型选型](#q4如何选择合适的-embedding-模型评估-embedding-好坏的指标有哪些)

**检索优化**
[Q5 提升检索质量的技术](#q5除了基础向量检索还有哪些可以提升-rag-检索质量的技术) · [Q6 Lost in the Middle 问题](#q6什么是lost-in-the-middle问题如何缓解) · [Q7 GraphRAG 适用场景](#q7什么场景下选择知识图谱graphrag而不是传统向量检索)

**评估与工程实践**
[Q8 RAG 系统全面评估](#q8如何全面评估一个-rag-系统从检索和生成两个阶段分别提指标) · [Q9 实际部署挑战](#q9rag-系统在实际部署中可能面临哪些挑战) · [Q10 开源 RAG 框架选型](#q10了解-ragflow-等开源-rag-框架吗如何选择合适的框架) · [Q11 搜索系统与 RAG 的区别](#q11搜索系统和-rag-有什么区别)

**大厂真题（高频）**
[Q12 时间衰减处理](#q12构建向量检索库时如何处理时间衰减对召回的影响) · [Q13 知识库增量更新](#q13知识库动态增量更新时如何避免新旧文档分布不一致导致的检索偏差) · [Q14 噪声处理](#q14rag-如果有噪声怎么办) · [Q15 BM25 算法原理](#q15讲一下-bm25-算法原理) · [Q16 意图识别](#q16意图识别是否做过如何实现) · [Q17 检索优化追问](#q17检索做了哪些优化追问子问题分解怎么做) · [Q18 三段式 Pipeline](#q18召回-过滤-生成三段式-pipeline-能细讲一下吗) · [Q19 Function Calling 与 MCP](#q19介绍-function-calling-和-mcp) · [Q20 高并发延迟优化](#q20高并发-agent-系统中如何优化召回和生成阶段的延迟) · [Q21 搜索引擎无关结果过滤](#q21让-agent-调用搜索引擎如何避免无关结果影响回答) · [Q22 自适应检索等高级 RAG 范式](#q22传统-rag-是先检索后生成你了解自适应检索等更复杂的-rag-范式吗)
